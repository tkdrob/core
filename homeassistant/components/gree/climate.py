...
class GreeClimateEntity(CoordinatorEntity, ClimateEntity):
    ...
    def __init__(self, coordinator):
        ...
        self._attr_temperature_unit = (
            TEMP_CELSIUS if units == TemperatureUnits.C else TEMP_FAHRENHEIT
        )
        self._attr_min_temp = (
            TEMP_MIN if self.temperature_unit == TEMP_CELSIUS else TEMP_MIN_F
        )
        self._attr_max_temp = (
            TEMP_MAX if self.temperature_unit == TEMP_CELSIUS else TEMP_MAX_F
        )
    ...
    async def async_turn_on(self) -> None:
        """Turn on the device."""
        _LOGGER.debug("Turning on HVAC for device %s", self.name)
    ...
    async def async_turn_off(self) -> None:
        """Turn off the device."""
        _LOGGER.debug("Turning off HVAC for device %s", self.name)
