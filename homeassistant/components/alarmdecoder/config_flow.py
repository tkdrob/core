...
class AlarmDecoderFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_protocol(self, user_input: dict | None = None) -> FlowResult:
...
def _validate_zone_input(zone_input: dict) -> dict:
    if not zone_input:
        return {}
    errors = {}
...
def _fix_input_types(zone_input: dict) -> dict:
...
def _device_already_added(
    ...
    user_host = user_input[CONF_HOST]
    user_port = user_input[CONF_PORT]
    user_path = user_input[CONF_DEVICE_PATH]
    user_baud = user_input[CONF_DEVICE_BAUD]
    entry: ConfigEntry
