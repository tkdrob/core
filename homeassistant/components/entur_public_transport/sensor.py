...
class EnturPublicTransportSensor(SensorEntity):
    ...
    _attr_icon = ICONS[DEFAULT_ICON_KEY]
    ...
    async def async_update(self) -> None:
        ...
        if self._show_on_map and data.latitude and data.longitude:
            self._attr_extra_state_attributes[CONF_LATITUDE] = data.latitude
            self._attr_extra_state_attributes[CONF_LONGITUDE] = data.longitude
        ...
        self._attr_state = due_in_minutes(calls[0].expected_departure_time)
        self._attr_icon = ICONS.get(calls[0].transport_mode, ICONS[DEFAULT_ICON_KEY])

        self._attr_extra_state_attributes[ATTR_ROUTE] = calls[0].front_display
        self._attr_extra_state_attributes[ATTR_ROUTE_ID] = calls[0].line_id
        self._attr_extra_state_attributes[ATTR_EXPECTED_AT] = calls[
            0
        ].expected_departure_time.strftime("%H:%M")
        self._attr_extra_state_attributes[ATTR_REALTIME] = calls[0].is_realtime
        self._attr_extra_state_attributes[ATTR_DELAY] = calls[0].delay_in_min

        number_of_calls = len(calls)
        if number_of_calls < 2:
            return

        self._attr_extra_state_attributes[ATTR_NEXT_UP_ROUTE] = calls[1].front_display
        self._attr_extra_state_attributes[ATTR_NEXT_UP_ROUTE_ID] = calls[1].line_id
        self._attr_extra_state_attributes[ATTR_NEXT_UP_AT] = calls[
            1
        ].expected_departure_time.strftime("%H:%M")
        self._attr_extra_state_attributes[
            ATTR_NEXT_UP_IN
        ] = f"{due_in_minutes(calls[1].expected_departure_time)} min"
        self._attr_extra_state_attributes[ATTR_NEXT_UP_REALTIME] = calls[1].is_realtime
        self._attr_extra_state_attributes[ATTR_NEXT_UP_DELAY] = calls[1].delay_in_min
