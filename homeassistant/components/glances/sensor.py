...
class GlancesSensor(SensorEntity):
    ...
    def __init__(
        ...
        self._attr_name = f"{name} {sensor_name_prefix} {sensor_name_suffix}"
        self._attr_icon = sensor_details[3]
        self._attr_unique_id = f"{glances_data.host}-{self.name}"
        self.type = sensor_type
        self.sensor_details = sensor_details
        self.unsub_update = None
        self._attr_device_class = sensor_details[4]
        self._attr_unit_of_measurement = sensor_details[2]
    ...
    async def async_update(self):  # noqa: C901
        ...
        self._attr_available = self.glances_data.available
        ...
        if self.sensor_details[0] == "fs":
            ...
            if self.type == "disk_free":
                try:
                    self._attr_state = round(disk["free"] / 1024 ** 3, 1)
                except KeyError:
                ...
                self._attr_state = round(disk["used"] / 1024 ** 3, 1)
            elif self.type == "disk_use_percent":
                self._attr_state = disk["percent"]
            ...
            self._attr_state = value["mem"]["percent"]
        elif self.type == "memory_use":
            self._attr_state = round(value["mem"]["used"] / 1024 ** 2, 1)
        elif self.type == "memory_free":
            self._attr_state = round(value["mem"]["free"] / 1024 ** 2, 1)
        elif self.type == "swap_use_percent":
            self._attr_state = value["memswap"]["percent"]
        elif self.type == "swap_use":
            self._attr_state = round(value["memswap"]["used"] / 1024 ** 3, 1)
        elif self.type == "swap_free":
            self._attr_state = round(value["memswap"]["free"] / 1024 ** 3, 1)
        elif self.type == "processor_load":
            # Windows systems don't provide load details
            try:
                self._attr_state = value["load"]["min15"]
            except KeyError:
                self._attr_state = value["cpu"]["total"]
        elif self.type == "process_running":
            self._attr_state = value["processcount"]["running"]
        elif self.type == "process_total":
            self._attr_state = value["processcount"]["total"]
        elif self.type == "process_thread":
            self._attr_state = value["processcount"]["thread"]
        elif self.type == "process_sleeping":
            self._attr_state = value["processcount"]["sleeping"]
        elif self.type == "cpu_use_percent":
            self._attr_state = value["quicklook"]["cpu"]
            ...
            try:
                for container in value["docker"]["containers"]:
                    ...
                    self._attr_state = round(cpu_use, 1)
            except KeyError:
            ...
            try:
                for container in value["docker"]["containers"]:
                    ...
                    self._attr_state = round(mem_use / 1024 ** 2, 1)
            except KeyError:
                self._attr_state = STATE_UNAVAILABLE
