...
async def test_low_battery_sensor(hass, multisensor_6, integration):
    ...
    assert state.attributes["device_class"] is BinarySensorDeviceClass.BATTERY
...
async def test_notification_sensor(hass, multisensor_6, integration):
    ...
    assert state
    assert state.state == STATE_ON
    assert state.attributes["device_class"] is BinarySensorDeviceClass.MOTION

    state = hass.states.get(TAMPER_SENSOR)

    assert state
    assert state.state == STATE_OFF
    assert state.attributes["device_class"] is BinarySensorDeviceClass.TAMPER
...
async def test_notification_off_state(
    ...
    door_states = [
        ...
        if state.attributes.get("device_class") is BinarySensorDeviceClass.DOOR
    ]
...
async def test_property_sensor_door_status(hass, lock_august_pro, integration):
    ...
    assert state
    assert state.state == STATE_OFF
    assert state.attributes["device_class"] is BinarySensorDeviceClass.DOOR
