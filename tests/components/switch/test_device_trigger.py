...
async def test_get_triggers(hass, device_reg, entity_reg):
    ...
    entity_reg.async_get_or_create(
        Platform.SWITCH, "test", "5678", device_id=device_entry.id
    )
    expected_triggers = [
        {
            "platform": "device",
            ...
            "entity_id": f"{Platform.SWITCH}.test_5678",
        },
        {
            "platform": "device",
            "domain": Platform.SWITCH,
...
async def test_if_fires_on_state_change(hass, calls, enable_custom_integrations):
    ...
    platform = getattr(hass.components, f"test.{Platform.SWITCH}")

    platform.init()
    assert await async_setup_component(
        hass, Platform.SWITCH, {Platform.SWITCH: {CONF_PLATFORM: "test"}}
    )
